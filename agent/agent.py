...
4. draft_reply
   ...
   The "context" field MUST include: plan name, seat counts, dollar amounts, discount rates, and
   specific facts from thread history. Do NOT use generic descriptions — include the actual numbers.
...
- The draft_reply context MUST include: current plan, current seats, price per seat, discount %, days remaining
- IMPORTANT: Only charge for NEW seats, not existing seats. Show calculation for incremental seats only.
- Show the actual calculation in the draft: (days_remaining / total_days) × per_seat_price × new_seats × (1-discount)
- Example: "5 new seats × $12/seat × (10/30) × 0.7 (30% discount) = $14"
- Do NOT just say "pro-rata billing applies" — show the numbers
...
class AgentReasoner:
    ...
    async def run(
        ...
        for step in range(1, MAX_STEPS + 1):
            ...
            # ── Post-parse safety override: block flag_for_legal on Complaint ──
            # The LLM occasionally confuses "angry customer" with "legal matter".
            # Hard-block this here so it can never happen regardless of LLM output.
            if action_name == "flag_for_legal" and (email.category or "") == "Complaint":
                logger.warning(
                    f"[Agent] Blocked flag_for_legal call on Complaint email {email_id} — "
                    "overriding to escalate_to_human."
                )
                action_name  = "escalate_to_human"
                action_input = {
                    "email_id":          email_id,
                    "reason":            "Complaint with potential churn risk — overrode erroneous legal flag",
                    "escalation_target": "customer_success@company.com",
                }
                # Patch the thought so the trace is honest
                thought = (
                    thought
                    + " [SAFETY OVERRIDE: flag_for_legal blocked for Complaint category; "
                    "redirected to escalate_to_human]"
                )
            ...
            if prior:
                ...
                if email.category in ("Billing", "Inquiry"):
                    base += (
                        ...
                        "current seat count, pricing, and any discounts. Include these exact numbers "
                        "in your draft_reply context — do not use placeholder text or policy links.\n"
                        "PRO-RATA RULE: For seat additions, only charge for NEW seats, not existing seats.\n"
                        "Example: 'Customer has 10 seats, adding 5 more → only charge pro-rata for 5 new seats.'\n"
                    )
